  ...
  if ($(`[aria-label="Post"]`).length) {
    ...
    if (stringSelected) {
      ...
      switch(type) {
        case 0:
          // code block
          reformatedText = stringSelected.replace(/[A-Za-z]/g, translate);
          break;
        case 3:
          reformatedText = stringSelected.replace(/[𝗔-𝗭𝗮-𝘇]/g, translate);
          // code block
          break;
        default:
          // reformatedText = stringSelected.replace(/[A-Za-z]/g, translate);
...
function translate(char)
{
    ...
    else if(/[𝗔-𝗭]/.test(char))
    {
        diff = "A".codePointAt(0) - "𝗔".codePointAt(0);
    }
    else if(/[𝗮-𝘇]/.test(char))
    {
        diff = "a".codePointAt(0) - "𝗮".codePointAt(0);
    }
